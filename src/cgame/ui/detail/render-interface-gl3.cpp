...
#include "render-interface-gl3.hpp"
...
RenderInterfaceGL3::RenderInterfaceGL3()
{
...
void RenderInterfaceGL3::SetViewport(int width, int height)
{
...
void RenderInterfaceGL3::EndFrame()
{
    // Restore GL state that RmlUi modifies so it doesn't leak into the game
    // renderer on the next frame (game renders before UI each frame).
...
Rml::CompiledGeometryHandle RenderInterfaceGL3::CompileGeometry(Rml::Span<const Rml::Vertex> vertices,
                                         ...
                                         Rml::TextureHandle texture)
{
    ...
    Gfx::CheckGLError("RenderGeometry");
}

void RenderInterfaceGL3::ReleaseGeometry(Rml::CompiledGeometryHandle handle)
{
    Gfx::CompiledGeometryData* geometry = (Gfx::CompiledGeometryData*)handle;
...
void RenderInterfaceGL3::EnableScissorRegion(bool enable)
{
...
void RenderInterfaceGL3::SetScissorRegion(Rml::Rectanglei region)
{
    const int x = region.Position().x;
    const int y = region.Position().y;
    const int width = region.Size().x;
    const int height = region.Size().y;
    ...
    if (transform_active)
    {
        ...
        Rml::CompiledGeometryHandle geom = CompileGeometry(
            Rml::Span<const Rml::Vertex>(vertices, 4),
            Rml::Span<const int>(indices, 6));
        if (geom)
        {
            RenderGeometry(geom, Rml::Vector2f(0, 0), 0);
...
Rml::TextureHandle RenderInterfaceGL3::LoadTexture(Rml::Vector2i& texture_dimensions, const Rml::String& source)
{
    ...
    Rml::TextureHandle handle = GenerateTexture(
        Rml::Span<const byte>(image_dest, image_size), texture_dimensions);
...
Rml::TextureHandle RenderInterfaceGL3::GenerateTexture(Rml::Span<const Rml::byte> source, Rml::Vector2i source_dimensions)
{
    GLuint texture_id = 0;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, internal_format, source_dimensions.x, source_dimensions.y, 0, GL_RGBA, GL_UNSIGNED_BYTE, source.data());
...
void RenderInterfaceGL3::SetTransform(const Rml::Matrix4f* new_transform)
{
...
void RenderInterfaceGL3::SubmitTransformUniform(ProgramId program_id, int uniform_location)
{
