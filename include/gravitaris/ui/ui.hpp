...
private:
    ...
    Rml::Element* m_capacitorFill = nullptr;
    Rml::Element* m_capacitorValue = nullptr;
    float m_capacitorFraction = -2.f;
    bool m_capacitorCharging = false;
    ...
    // shield or a capacitor belongs to exactly one slot on the drawing.
...
public:
    ...
    // The charge bank's readout: `fraction` is how much is left in it (1 full,
    // 0 just spent), `charging` whether that is the bank filling back up
    // rather than something drawing it down. Negative blanks the row, for a
    // ship that hasn't collected the upgrade.
    void SetCapacitorReadout(float fraction, bool charging);
