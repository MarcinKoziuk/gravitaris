...
damage landed on the hull — which at the time only regenerated by standing on
one of your own developed planets (`RepairSystem`). So the plating pilot
accumulated permanent damage the bubble pilot never takes, and the bigger
reservoir and faster regen did not pay for it. The hull regen below is the
answer to that; 1b is still open.

**Done: the hull mends under plating.** Plating now carries
`hull_regen_seconds = [90, 45, 30]` (`data/upgrades.toml`), resolved to
`ShipStats::hullRegenFractionPerSecond` and applied by
`RepairSystem::MendUnderPlating` — a share of `maxHp` per second, anywhere,
landed or not. That is the leak's counterweight: what gets through plating is
no longer permanent the way what gets through a bubble is, so the plated pilot
does not have to fly home for every unlucky round. A fraction rather than hit
points because the plates wrap whatever hull they are fitted to. The bubble
authors no key and so mends nothing, which needs no type check in code.

Deliberately *not* gated on a quiet period after a hit — at a full hull in 30
seconds it is far too slow to matter under fire, and a delay would be a second
timer to tune for no gain. Revisit if the rate is ever raised.

### 1a. Plates absorb collisions and hard landings — WITHDRAWN 2026-08-09

Dropped, not deferred, once the plumbing was read properly. Two reasons, both
worth knowing before anyone re-proposes it:

- **The plate lookup does not exist.** The plan was to resolve which plate took
  a collision via `DamageSystem::ShieldElementFor`, but that takes a `cpShape*`
  and neither `ImpactEvent` nor `ShipRamEvent`
  (`include/gravitaris/game/system/core/physics-system.hpp:73`) carries one —
  only a world-space contact point. It would have needed a new point→plate
  query or a widened event.
- **Ramming has no damage number to absorb.** The ram path is a *kill* model:
  `kill()` sets `hp = 0` outright, and an evenly-matched or high-momentum pair
  both die. Only the survivor's residual damage is a value a shield could eat,
  so "plating makes a ram survivable" would have meant changing the toughness
  comparison, which is a different and larger change than routing damage.

The hull regen above was taken instead, as the same perk reached a cheaper way.
