...
LineMiterShader::LineMiterShader(IFilesystem& fileSystem)
{
    ...
    if (!fileSystem.ReadString("shaders/line-miters.v.glsl", &vertexSource)) {
    ...
    bindAttributeLocation(LineMiterShader::Position::Location, "position");
    bindAttributeLocation(LineMiterShader::InstancePointA::Location, "instancePointA");
    bindAttributeLocation(LineMiterShader::InstancePointB::Location, "instancePointB");
    bindAttributeLocation(LineMiterShader::InstancePointC::Location, "instancePointC");
...
LineMiterShader& LineMiterShader::setWidth(Magnum::Float width)
{
    setUniform(u_width, width);
    return *this;
}

LineMiterShader& LineMiterShader::setColor(const Magnum::Color3& color)
{
    setUniform(u_color, color.toSrgb());
    return *this;
}

LineMiterShader& LineMiterShader::setTransformationProjectionMatrix(const Matrix3& matrix)
{
